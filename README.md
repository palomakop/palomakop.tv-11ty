...
## Automatic archiving of event pages

If a build is triggered after 5am the next day after an event's date (or end date if it has one), the event will automatically be set to archived, unless explicitly overridden with a boolean flag in the frontmatter (i.e. `archived: True`). An archived event doesn't show up in "upcoming events", and a note is added to the top of the event page.

If an event is archived due to its date, any updates on that event page be set to archived (and stop them from showing on the main index and /now pages). This can also be overridden with an explicit boolean in the update yaml.
